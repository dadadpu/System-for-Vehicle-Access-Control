...
int CPlateLocate::colorSearch(const Mat &src, const Color r, Mat &out,
  ...
  //if (m_debug) {
  //  utils::imwrite("resources/image/tmp/color.png", src_threshold);
  //}
...
int CPlateLocate::sobelSecSearchPart(Mat &bound, Point2f refpoint,
  ...
  if (bFindLeftRightBound(tempBoundThread, posLeft, posRight)) {
    ...
    utils::imwrite("resources/image/tmp/repaireimg1.png", bound_threshold);
    ...
    utils::imwrite("resources/image/tmp/repaireimg2.png", bound_threshold);
  }
...
int CPlateLocate::sobelSecSearch(Mat &bound, Point2f refpoint,
  ...
  utils::imwrite("resources/image/tmp/sobelSecSearch.png", bound_threshold);
...
void deleteNotArea(Mat &inmat, Color color = UNKNOWN) {
  ...
  if (BLUE == plateType) {
    ...
    utils::imwrite("resources/image/tmp/inputgray2.png", img_threshold);
...
int CPlateLocate::sobelOperT(const Mat &in, Mat &out, int blurSize, int morphW,
  ...
  utils::imwrite("resources/image/tmp/grayblure.png", mat_gray);
  ...
  utils::imwrite("resources/image/tmp/graygrad.png", grad);

  Mat mat_threshold;
  double otsu_thresh_val =
      threshold(grad, mat_threshold, 0, 255, CV_THRESH_OTSU + CV_THRESH_BINARY);

  utils::imwrite("resources/image/tmp/grayBINARY.png", mat_threshold);

  Mat element = getStructuringElement(MORPH_RECT, Size(morphW, morphH));
  morphologyEx(mat_threshold, mat_threshold, MORPH_CLOSE, element);

  utils::imwrite("resources/image/tmp/phologyEx.png", mat_threshold);
